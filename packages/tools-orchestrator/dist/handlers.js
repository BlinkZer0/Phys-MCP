...
export async function handleExperimentOrchestratorTool(toolName, params) {
    ...
    if (toolName !== 'experiment_orchestrator') {
        ...
        if (toolName === 'define_dag') {
            ...
            method = 'validate_dag';
            actualParams = { ...restParams, method: 'validate_dag' };
        }
        else if (toolName === 'run_dag') {
            method = 'run_dag';
            actualParams = { ...restParams, method: 'run_dag' };
        }
        else if (toolName === 'publish_report') {
            method = 'publish_report';
            actualParams = { ...restParams, method: 'publish_report' };
        }
        else if (toolName === 'collaborate_share') {
            method = 'collaborate_share';
            actualParams = { ...restParams, method: 'collaborate_share' };
        }
        else {
